...
class StackedGeneralizer(object):
    def __init__(self, clfs=None, ensemble=None, n_folds=5, stratify = False, original = False, epoch=10, batch_size=128):
        self.clfs = clfs
        ...
        for j, clf in enumerate(self.clfs):
            ...
            if type(clf) == keras.models.Sequential:
                for i, (train, test) in enumerate(kf.split(X_train, y)):
                    print('Fold %d' % (i + 1))
                    clf.fit(X_train[train], pd.get_dummies(y[train]).as_matrix(), nb_epoch=self.epoch, batch_size=self.batch_size, verbose=0)
